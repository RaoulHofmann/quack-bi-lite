<template>
  ...
  >
    <input type="file" accept=".csv,.xlsx,.xls,.json" class="hidden" ref="inputRef" @change="onFile" />
    <div class="space-y-4">
      ...
      <div>
        <p class="text-gray-600 font-medium">Drop a file here</p>
        <p class="text-gray-400 text-sm mt-1">or click to browse</p>
      </div>
      <p class="text-xs text-gray-400">Supports CSV, Excel (.xlsx), and JSON files. All data stays on your device.</p>
    </div>
...
function validateAndEmit(file) {
  ...
  if (!allowedExtensions.includes(ext)) {
    error.value = 'Unsupported file type. Please upload CSV, Excel (.xlsx), or JSON files.'
    return
  }
